...
static int ListMonitors()
{
    ...
    foreach (var monitor in monitors)
    {
        ...
        table.AddRow(
            monitor.IsPrimary ? $"[bold]{monitor.Index}[/] ⭐" : monitor.Index.ToString(),
            monitor.Name,
            monitor.DeviceName,
            inputInfo,
            status);
...
static int SetInput(string[] args)
{
    ...
    AnsiConsole.Status()
        .Start($"Switching {monitor.Name} to {InputSource.GetName(inputValue)}...", ctx =>
        {
            ...
            {
                throw new Exception($"Failed to set input source on monitor '{monitor.Name}'. The monitor may not support this input or requires administrator privileges.");
            }
